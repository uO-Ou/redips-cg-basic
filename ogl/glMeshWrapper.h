/*
* opengl Mesh Wrapper
* if (setup_type = true) groups with same material pack into a vbo
...
public:
	glMeshWrapper(const Triangles* model,bool setup_type = true){
		...
		if (setup_type){// check if groups with same mtl have same face-type
	...
	glMeshWrapper(const glMeshWrapper& another){
		...
		this->textureCnt = another.textureCnt;
