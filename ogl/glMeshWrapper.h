...
public:
	glMeshWrapper(const Mesh& mesh){
		...
		for (int i = 0; i < meshCnt; i++){
			...
			glVertexAttribPointer(1, 3, GL_FLOAT, GL_FALSE, 0, (const GLvoid*)(sizeof(float)*(mesh.groups[i].faceCnt * 3 * (mesh.groups[i].faceType == _withtex_ ? 5 : 3))));
		}
