...
#define _DIRECTION_CNT_PER_BLOCK_ 32u           // _DIRECTION_CNT_PER_BLOCK_*16 must less than _THREAD_NUM_PER_BLOCK_
#define _THREAD_NUM_PER_BLOCK_ 512
#define _TRANSFORM_EPSI_ 0.38f       //garden 0.38/0.38  trees 0.22/0.38 sponza 0.5/0.8
...
__inline__ __device__ float length(const float3& vec){
...
__global__ void transform(float4* frags, int fragcnt, redips::float3 center, float* mats, float* axises, float3* radius, unsigned int sid,unsigned int eid,unsigned int precision,unsigned int * result){
	...
	__shared__ float axises_sm[_DIRECTION_CNT_PER_BLOCK_*9];
	__shared__ float3 radius_sm[_DIRECTION_CNT_PER_BLOCK_];
	__shared__ float epsies_sm[_DIRECTION_CNT_PER_BLOCK_];
	{
		float* matsptr = mats + ((sid+matSId) << 4);
		float* axisesptr = axises + ((sid + matSId) * 9);
		if (tid < (matCnt << 4u)){
			mats_sm[tid] = matsptr[tid];
			if (tid < ((matCnt << 3u) + matCnt)){
				axises_sm[tid] = axisesptr[tid];
				if (tid < matCnt){
					radius_sm[tid] = radius[sid + matSId + tid];
					epsies_sm[tid] = length(radius_sm[tid]) / res2 * _TRANSFORM_EPSI_;
				}
			}
		}
		__syncthreads();
	...
	for (; tid < fragcnt; tid += _THREAD_NUM_PER_BLOCK_){
		...
		for (unsigned int mid = 0; mid < matCnt; mid++){  // mid+sid 
			...
			float xx = ((0.5f + tx) / res2 - 1) * radius_sm[mid].x;
			float yy = ((0.5f + ty) / res2 - 1) * radius_sm[mid].y;
			float zz = ((0.5f + tz) / res2 - 1) * radius_sm[mid].z;

			float xdiff = xx*axisesptr[0] + yy*axisesptr[3] + zz*axisesptr[6] - dot.x;
			float ydiff = xx*axisesptr[1] + yy*axisesptr[4] + zz*axisesptr[7] - dot.y;
			float zdiff = xx*axisesptr[2] + yy*axisesptr[5] + zz*axisesptr[8] - dot.z;

			if (sqrt(xdiff*xdiff + ydiff*ydiff + zdiff*zdiff) < epsies_sm[mid]) {
					//if (blockIdx.x == 0) printf("%f %f yes\n", xdiff*xdiff + ydiff*ydiff + zdiff*zdiff, epsies_sm[mid]);
...
extern "C"
void mdVoxelization_cuda(GLuint fragsvbo, unsigned int fragscnt, redips::float3 fragscenter, int rotx, int roty, const float* mats, const float* axises, const float* radius, unsigned int presicion,unsigned int* cpumem,std::string outputdir){
	   //prepare
	   ...
	   if (DIRECTION_CNT_PER_PROCESS > rotx*roty)  DIRECTION_CNT_PER_PROCESS = rotx*roty;
	   ...
	   if (cpumem != nullptr) rbuffer = cpumem;
	   else rbuffer = new unsigned int[result_uint_cnt];
	   char strBuffer[256];  
	   if (outputdir.length() > 0){
		   sprintf(strBuffer, "%s/%d", outputdir.c_str(), (1u << presicion)); outputdir = std::string(strBuffer) + "/";
		   _mkdir(strBuffer);
	   }
	   
	   //allocate memory
	   float* frags_dev=nullptr; size_t num_bytes;
	   ...
	   float* axises_dev = nullptr;
	   checkCudaErrors(cudaMalloc((void**)&axises_dev,sizeof(float)*9*rotx*roty));
	   checkCudaErrors(cudaMemcpy(axises_dev,axises,sizeof(float)*9*rotx*roty,cudaMemcpyHostToDevice));

	   float* radius_dev=nullptr;
	   checkCudaErrors(cudaMalloc((void**)&radius_dev, sizeof(float)* 3 * rotx*roty));
	   checkCudaErrors(cudaMemcpy(radius_dev, radius, sizeof(float)* 3 * rotx*roty, cudaMemcpyHostToDevice));

	   unsigned int* result_dev=nullptr;
	   ...
	   for (int sid = 0; ;sid += DIRECTION_CNT_PER_PROCESS){
		   ...
		   CC(cudaDeviceSynchronize());
		   transform << <blockCnt, _THREAD_NUM_PER_BLOCK_ >> >((float4*)frags_dev, fragscnt, fragscenter, mats_dev, axises_dev,(float3*)radius_dev,sid, tid, presicion, result_dev);
		   CC(cudaDeviceSynchronize());
	   ...
	   if(cpumem==nullptr) delete rbuffer;
	   sdkDeleteTimer(&timer);
	   checkCudaErrors(cudaFree(result_dev));
	   checkCudaErrors(cudaFree(mats_dev));
	   checkCudaErrors(cudaFree(axises_dev));
	   checkCudaErrors(cudaFree(radius_dev));
